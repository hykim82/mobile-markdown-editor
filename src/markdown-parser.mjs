...
// `s`(dotAll) 플래그(HYK-304-linebreak-1 E4): restore.mjs 가 한 문단/항목
// 안 Shift+Enter 줄바꿈을 "\\\n"(backslash + 실제 줄바꿈 한 글자)으로 되돌릴
// 자리를 찾으려고, prefix 판정 대상 문자열에 그 실제 줄바꿈 글자가 그대로
// 남아 들어올 수 있다 -- `s` 없인 `.`가 줄바꿈을 건너뛰지 못해 `$`(문자열
// 끝)까지 못 가 매치 자체가 실패하고, 타입 판정(헤딩/목록)이 조용히
// "문단"으로 잘못 떨어진다.
const HEADING_RE = /^(#{1,3}) (.*)$/su;
const CHECKBOX_RE = /^- \[( |x)\] (.*)$/su;
const BULLET_RE = /^- (.*)$/su;
