...
function applyRecordToEditor(
  ...
  // 리베이스(#6 위, HYK-304-linebreak-3): 옛 형식 메모는 restore/serialize
  // 양쪽에 legacy 옵션을 걸어야 "보이는 글자"가 원문과 같아진다 --
  // isLegacyRecord 주석 참조. record 가 없으면(새 메모) 현재 형식이므로
  // legacy=false 다. 이 한 줄이 빠지면 옛 메모의 역슬래시가 두 개로 보인다.
  const legacy = record ? isLegacyRecord(record) : false;
  restoreState.applying = true;
  restoreFn(editor, record ? record.body : "", { legacy });
